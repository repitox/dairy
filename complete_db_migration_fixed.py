        ...
        with conn.cursor() as cur:
            try:
                ...
                if not has_id_column:
                    ...
                    # Сначала проверим, есть ли уже PRIMARY KEY
                    cur.execute("""
                        SELECT constraint_name FROM information_schema.table_constraints 
                        WHERE table_name = 'users' AND constraint_type = 'PRIMARY KEY'
                    """)
                    has_primary_key = cur.fetchone()
                    
                    if has_primary_key:
                        print("  ⚠️  Удаляем существующий PRIMARY KEY...")
                        cur.execute(f"ALTER TABLE users DROP CONSTRAINT {has_primary_key['constraint_name']}")
                ...
                for table in tables_to_update:
                    try:
                        cur.execute(f"SELECT id, user_id FROM {table} WHERE user_id > 1000000 LIMIT 100")
