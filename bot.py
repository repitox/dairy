...
async def on_startup():
    ...
    if not os.path.exists(marker_file):
        ...
        # Создаём пустой маркер-файл
        try:
            with open(marker_file, "w") as f:
                pass
            print(f"Маркер-файл создан: {marker_file}")
        except Exception as e:
            print(f"❌ Не удалось создать маркер-файл: {e}")
    else:
        print(f"Маркер-файл уже существует: {marker_file}. Сообщение не отправляем.")
