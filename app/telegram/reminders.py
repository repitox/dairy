    ...
    while True:
        ...
        for event in events:
            for user_id in users:
                ...
                # Парсим UTC дату из БД и форматируем для пользователя
                try:
                    ...
                    print(
                        f"Ошибка форматирования времени события {event['id']} для пользователя {user_id}: {e}"
                    )
                    # Fallback к старому способу
                    try:
                        start = datetime.fromisoformat(event["start_at"])  # может быть без Z
                        offset_hours = (
                            int(user_tz_offset)
                            if isinstance(user_tz_offset, str)
                            and (user_tz_offset.lstrip("+-").isdigit())
                            else 0
                        )
                ...
                try:
                    ...
                    _log_event(
                        "reminder",
                        f"Уведомление отправлено пользователю {user_id} о событии '{event['title']}'",
                    )
                    _record_reminder_sent(user_id, event["id"])
                except Exception as e:
                    _log_event(
                        "error", f"Ошибка при отправке пользователю {user_id}: {type(e).__name__} — {e}"
                    )
